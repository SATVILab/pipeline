test_that("save_objects works", {
  ...
  expect_identical(readRDS(file.path(dir_proj, "test", "x.rds")),
                   x)
  expect_identical(readRDS(file.path(dir_proj, "test", "y.rds")),
                   y)

  # test that emptying the directory works
  z <- 3
  save_objects(z = 2, dir_proj = dir_proj, dir_sub = "test",
                empty = TRUE)
  expect_true(file.exists(file.path(dir_proj, "test", "z.rds")))
  expect_true(!file.exists(file.path(dir_proj, "test", "x.rds")))

  # test that saving using obj_list and ... works
  x <- 1; obj_list <- list('y' = 4)
  save_objects(x = x, obj_list = obj_list,
  ...
  v <- 1
  save_objects(v = v, dir_proj = dir_proj, dir_sub = "test",
  ...
  v <- 1; obj_list <- list(2)
  save_objects(v, obj_list = obj_list, dir_proj = dir_proj,
  ...
  obj_list <- list(z = 3)
  save_objects(obj_list = obj_list, dir_proj = dir_proj,
  ...
  expect_true(!file.exists(file.path(dir_proj, "test",
                                    "x-uni_3.rds")))
  expect_identical(readRDS(file.path(dir_proj, "test",
                                     "x-uni_1.rds")),
                   1)
  expect_identical(readRDS(file.path(dir_proj, "test",
                                     "x-uni_2.rds")),
  ...
  save_objects(x = 1, dir_proj = dir_proj,
  ...
  save_objects(p = p, x = 1, dir_proj = dir_proj,
  ...
  save_objects(p, x = 1, dir_proj = dir_proj,
                dir_sub = c("test"), silent = TRUE)
  expect_true(file.exists(file.path(dir_proj, 'test',
                                    "obj_1.png")))
  expect_true(file.exists(file.path(dir_proj, 'test',
